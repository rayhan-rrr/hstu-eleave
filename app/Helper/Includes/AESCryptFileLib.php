...
class AESCryptFileLib
{
	...
	public function __construct(AES256Implementation $aes_impl, $use_dynamic_filenaming = true) {
		...
		try {
			...
			echo "AESCryptMissingDependencyException";
		}
	}
	...
	public function readExtensionBlocks($source_file)
	{
		...
		if ($source_fh === false) {
			...
			echo "AESCryptFileAccessException";
		}
			...
			while (true) {
				...
				if ($ext_length == 0) {
					...
					if ($null_index === false) {
						...
						echo "AESCryptCorruptedFileException";
					}
	...
	private function checkSourceExistsAndReadable($source_file)
	{
		...
		if (!file_exists($source_file)) {
			...
			echo "AESCryptFileMissingException";
		}
	...
	private function openDestinationFile($source_file, $dest_file, $encrypting = true) {
		...
		if ($dest_file === NULL) {
			if (!$encrypting) {
				...
				if (preg_match("/^(.+)\." . self::ENCRYPTED_FILE_EXTENSION . "$/i", $source_info['basename'], $matches)) {
					...
					echo "AESCryptCannotInferDestinationException";
				}
		...
		if ($this->use_dynamic_filenaming) {
			...
			if (file_exists($dest_file)) {
				...
				echo "AESCryptFileExistsException";
			}
		}
	...
	private function checkExtensionData($ext_data)
	{
		...
		if (!is_array($ext_data)) {
			...
			echo "AESCryptInvalidExtensionException";
		}
	...
	private function checkPassphraseIsValid($passphrase)
	{
		if ($passphrase === NULL) {
			...
			echo "AESCryptInvalidPassphraseException";
		}
	}
	...
	private function doEncryptFile($source_file, $passphrase, $dest_file, $ext_data)
	{
		...
		if (self::bin_strlen($iv_1) != 16) {
			...
			echo "AESCryptImplementationException";
		}
		...
		if (self::bin_strlen($enc_key_1) != 32) {
			...
			echo "Returned a passphrase which is not 32 bytes long";
		}
		...
		if (self::bin_strlen($iv_2) != 16) {
			...
			echo "Returned an IV which is not 16 bytes long";
		}
		...
		if (self::bin_strlen($enc_key_2) != 32) {
			...
			echo "Returned a random key which is not 32 bytes long";
		}
		...
		if ($written === false) {
			...
			echo "Could not write encrypted data to file.  Tried to write ";
		}
	...
	private function doDecryptFile($source_file, $passphrase, $dest_file)
	{
		...
		if ($version_chunk === 0) {
			...
			if ($result === false) {
				...
				echo "Could not write back file";
			}
	...
	private function validateHMAC($key, $data, $hash, $name)
	{
		...
		if ($calculated != $actual) {
			...
			if ($name == "HMAC 1") {
				...
				echo "failed to validate integrity of encryption keys.  Incorrect password or file corrupted";
			} else {
				...
				echo "failed to validate integrity of encrypted data.  The file is corrupted and should not be trusted";
			}
		}
	}
